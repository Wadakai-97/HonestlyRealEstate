...
<table class="edit_menu">
    <tbody>
        <td><a href="{{ route('admin.newDetachedHouseGroup.edit', ['id' => $new_detached_house->id]) }}" class="edit_menu_link">物件概要</a></td>
        <td><a href="{{ route('admin.newDetachedHouseGroupImage.edit', ['id' => $new_detached_house->id]) }}" class="edit_menu_link">物件画像</a></td>
    </tbody>
</table><br>
        ...
        <tr class="property_images">
            @foreach($new_detached_house_images as $new_detached_house_image)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.newDetachedHouseGroupImage.update', ['id' => $new_detached_house_image->id]) }}" enctype="multipart/form-data">
                        @csrf
                    ...
                    <form action="{{ route('admin.newDetachedHouseGroupImage.delete', ['id' => $new_detached_house_image->id]) }}">
            ...
            @while($image_counter < 21)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.newDetachedHouseGroupImage.signUp', ['id' => $new_detached_house->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
