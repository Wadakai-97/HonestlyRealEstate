...
<head>
                    ...
                    $.each(data, function(key, val) {
                        html = `
                            <tr class="mansion">
                                ...
                                <td class="combine_td_left"><button id="mansionDetail" type="button" onclick="mansionDetail()">詳細</button></td>
                                <td class=”combine_td_right"><form method="POST" action="/public/mansion/delete/${val.id}">@csrf<input type="submit" id="mansionDelete" value="削除"></form></td>
                            </tr>`
    ...
    <script>
        $(function() {
            $(document).on('click', '#mansionImageDelete', function() {
                var delete_confirm = confirm('物件画像を削除しますか？');
                var id = document.getElementById('mansionImageId').value();
                var image_counter = document.getElementById('iimageCounter').value();
                ...
                if(delete_confirm == true) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "/public/mansion/delete/image/" + id,
                        data: {"id": id,
                                "_method": "delete",}
                    }).done(function() {
                        var reset_image = document.getElementById("showImage");
                        reset_image.setAttribute("src", "/public/storage/mansion_images/no_image.jpeg");
                        alert('物件画像の削除に成功しました。');
                    }).fail(function(jqXHR, textStatus, errorThrown) {
                        console.log("jqXHR      :" + jqXHR.status);
                        console.log("textStatus :" + textStatus);
                        console.log("errorThrown:" + errorThrown.message);
                        alert('物件画像の削除に失敗しました。')
                    });
                } else {
                    (function(e) {
                        e.preventDefault()
