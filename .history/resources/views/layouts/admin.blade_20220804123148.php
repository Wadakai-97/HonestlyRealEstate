...
<script type="text/javascript">
    function state_select() {
     ...
     var selecttownoptions = selecttown.options
     while (0 < selecttownoptions.childNodes.length) {
         selecttownoptions.removeChild(selecttownoptions.childNodes[0]);
     }
     selecttown.options[0] = new Option('↓(町域)','');
     selecttown.value = "";
     var httpObj = jQuery.get("./citytext.php?state=" + frm.state.value,null, function(){
         rspdata = httpObj.responseText
         //レンタルサーバの広告部分を取り除くため区切り記号を検索しデータ部分を確定する（区切り記号は/////）
         wstate=rspdata.split("/////");
         wstate2=wstate[1];
      var selectcity = document.getElementById("city");
      while (0 < selectcity.childNodes.length) {
          selectcity.removeChild(selectcity.childNodes[0]);
      }
      frm.city.options[0] = new Option('↓(市区町村)','');
         optline=1;
         optdata = wstate2.split(";");
      for (var i=0;i<optdata.length;i++) {
       wopt=optdata[i].split(",");
       if (wopt.length == 2) {
        frm.city.options[optline] = new Option(wopt[0],wopt[0] + ',' + wopt[1]);
        optline++;
       }
      }
      frm.city.value="";
    ...
    function city_select() {
     //市区町村のデータが「表示文字,APIに渡すURL」で格納しているのでURLデータをとり出す
     wcity=frm.city.value;
     wcity2=wcity.split(',');
     //非同期通信にて町域APIからデータを取得する
     //PHPにgetパラメータを含むURLデータを送る場合にはencodeURIComponent関数を使用してエンコードする
     var httpObj = jQuery.get("./towntext.php?townid=" + encodeURIComponent(wcity2[1]),null, function(){
         rspdata = httpObj.responseText
         //レンタルサーバの広告部分を取り除くため区切り記号を検索しデータ部分を確定する（区切り記号は/////）
         wcity=rspdata.split("/////");
         wcity2=wcity[1];
         //町域の内容を書き換えるため町域のプルダウンの要素を全て取り除く
      var selecttown = document.getElementById("town");
      while (0 < selecttown.length) {
          selecttown.remove(0);
      }
      // 新しい町域データを追加する
      ...
      try {
       //IE用
       selecttown.add(newOption);
      } catch (e) {
       //その他ブラウザ用
       selecttown.appendChild(newOption);
      }
         optdata = wcity2.split(";");
      for (var i=0;i<optdata.length;i++) {
       if (optdata[i] != "") {
        newOption = document.createElement("option");
        newOption.value = optdata[i];
        newOption.text=optdata[i];
        try {
         //IE用
         selecttown.add(newOption);
        } catch (e) {
