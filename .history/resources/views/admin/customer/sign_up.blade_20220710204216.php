    ...
    <table>
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('emai'))) has-error @endif">
                        <select name="emai">
                            <option disabled selected>未選択</option>
                            @foreach (StaffConsts::emai_LIST as $value => $emai)
                            <option value="{{ $value }}" @if(old('emai') === '{{ $value }}') selected @endif>{{ $emai }}</option>
                            @endforeach
                        </select>
                        <span class="help-block">{{$errors->first('emai')}}</span>
                    </div>
                </td>
            </tr>
            <tr>
