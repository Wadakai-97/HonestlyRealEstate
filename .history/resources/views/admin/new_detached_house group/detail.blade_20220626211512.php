...
@section('title', '新築分譲住宅：物件詳細')
@section('body')
<h2>{{ $new_detached_house_group->name }}：詳細</h2>
<table>
    <tbody>
        <tr>
        ...
        <tr>
            ...
            <th>販売価格</th>
            <td>
                {{ $new_detached_house_group->price}}万円（{{ $new_detached_house_group->tax }}）
            </td>
        </tr>
        <tr>
            <th>住所</th>
        ...
        <tr>
            <th>土地面積</th>
            <td>
                {{ $new_detached_house_group->land_area }}㎡
            </td>
            <th>建物面積</th>
            <td>
                {{ $new_detached_house_group->building_area }}㎡
            </td>
        </tr>
        <tr>
            <th>間取り</th>
            <td>
                {{ $new_detached_house_group->number_of_rooms }}{{ $new_detached_house_group->type_of_room }}
            </td>
        </tr>
        <tr>
            <th>方角</th>
            ...
            <th>バルコニー面積</th>
            <td>
                {{ $new_detached_house_group->balcony_area }}㎡
            </td>
        </tr>
        <tr>
            <th>完成時期（築年月）</th>
        ...
        <tr>
            ...
            <th>駐車場</th>
            <td>
                {{ $new_detached_house_group->parking_lot }}
            </td>
        </tr>
        <tr>
            <th>用途地域</th>
...
<form action="{{ route('admin.newDetachedHouse.groupEdit', ['id' => $new_detached_house_group->id]) }}">
    @csrf
