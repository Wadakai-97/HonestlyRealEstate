...
<form method="post" action="{{ route('admin.newDetachedHouseGroup.save', ['id' => $new_detached_house->id]) }}" enctype="multipart/form-data">
    @method('patch')
    @csrf
    <table>
        <thead>
        ...
        <tbody>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('price'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('tax'))) has-error @endif">
                            <input type="number" name="price" value="{{ old('price', $new_detached_house->price) }}">万円
                            <select name="tax">
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('pref'))) has-error @endif">
                        <input type="text" id="都道府県" name="pref" placeholder="〇〇県" value="{{ old('pref', $new_detached_house->pref) }}">
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('land_area'))) has-error @endif">
                        <input type="number" name="land_area" value="{{ old('land_area', $new_detached_house->land_area) }}">㎡
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('building_area'))) has-error @endif">
                        <input type="number" name="building_area" value="{{ old('building_area', $new_detached_house->building_area) }}">㎡
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('balcony_area'))) has-error @endif">
                        <input type="number" name="balcony_area" value="{{ old('balcony_area', $new_detached_house->balcony_area) }}">㎡
                    ...
                    <div class="form-group @if(!empty($errors->first('number_of_rooms'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('type_of_room'))) has-error @endif">
                            ...
                            <select name="type_of_room">
                                <option disabled selected>未選択</option>
                                <option value="K" @if(old('type_of_room', $new_detached_house->type_of_room) === 'K') selected @endif>K</option>
                                <option value="DK" @if(old('type_of_room', $new_detached_house->type_of_room) === 'DK') selected @endif>DK</option>
                                <option value="LDK" @if(old('type_of_room', $new_detached_house->type_of_room) === 'LDK') selected @endif>LDK</option>
                            </select>
            ...
            <tr>
                ...
                <td>
                    <select name="parking_lot">
                        <option disabled selected>未選択</option>
                        <option value="なし" @if(old('parking_lot', $new_detached_house->parking_lot) === 'なし') selected @endif>なし</option>
                        <option value="１台" @if(old('parking_lot', $new_detached_house->parking_lot) === '１台') selected @endif>１台</option>
                        <option value="２台" @if(old('parking_lot', $new_detached_house->parking_lot) === '２台') selected @endif>２台</option>
                        <option value="３台" @if(old('parking_lot', $new_detached_house->parking_lot) === '３台') selected @endif>３台</option>
                        <option value="４台" @if(old('parking_lot', $new_detached_house->parking_lot) === '４台') selected @endif>４台</option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('status'))) has-error @endif">
                        <select name="status">
                            <option disabled selected>未選択</option>
                            <option value="更地" @if(old('status', $new_detached_house->status) === '更地') selected @endif>更地</option>
                            <option value="建築中" @if(old('status', $new_detached_house->status) === '建築中') selected @endif>建築中</option>
                            <option value="完成済み" @if(old('status', $new_detached_house->status) === '完成済み') selected @endif>完成済み</option>
                        </select>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('adjacent_road_width'))) has-error @endif">
                        <input type="text" name="adjacent_road_width" value="{{ old('adjacent_road_width', $new_detached_house->adjacent_road_width) }}">ｍ
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('setback'))) has-error @endif">
                        <input type="text" name="setback" value="{{ old('setback', $new_detached_house->setback) }}" placeholder="">
            ...
            <tr>
                ...
                <td>
                    <input type="number" name="walking_distance_station" value="{{ old('walking_distance_station', $new_detached_house->walking_distance_station) }}">分
                </td>
            </tr>
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('conditions_of_transactions'))) has-error @endif">
                        <select name="conditions_of_transactions">
                            ...
                            <option value="{{ $key }}" @if(old('conditions_of_transactions', $new_detached_house->onditions_of_transactions) === $conditions_of_transactions) selected @endif>{{ $conditions_of_transactions }}</option>
                            @endforeach
                        </select>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <th><p>セールスコメント</p></th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="sales_comment">{{ old('sales_comment', $new_detached_house->sales_comment) }}</textarea>
                </td>
            </tr>
            <tr>
                <th><p>物件紹介コメント</p></th>
                <td>
                    <textarea maxlength="800" placeholder="最大800文字まで" name="property_introduction">{{ old('property_introduction', $new_detached_house->property_introduction) }}</textarea>
                </td>
            </tr>
            <tr>
                <th><p>設備条件</p></th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="terms_and_conditions">{{ old('terms_and_conditions', $new_detached_house->terms_and_conditions) }}</textarea>
                </td>
            </tr>
        </tbody>
    </table>
