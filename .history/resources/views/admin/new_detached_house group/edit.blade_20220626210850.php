...
<form method="post" action="{{ route('admin.newDetachedHouseGroup.save', ['id' => $っｄ->id]) }}" enctype="multipart/form-data">
    @method('patch')
    @csrf
    <table>
        <thead>
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('name'))) has-error @endif">
                        <input type="text" name="name" value="{{ old('name', $っｄ->name) }}">
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('price'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('tax'))) has-error @endif">
                            <input type="number" name="price" value="{{ old('price', $っｄ->price) }}">万円
                            <select name="tax">
                                <option disabled selected>未選択</option>
                                <option type="text" value="税込" @if(old('tax', $っｄ->tax) === '税込') selected @endif>税込</option>
                                <option type="text" value="非課税" @if(old('tax', $っｄ->tax) === '非課税') selected @endif>非課税</option>
                            </select>
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('pref'))) has-error @endif">
                        <input type="text" id="都道府県" name="pref" placeholder="〇〇県" value="{{ old('pref', $っｄ->pref) }}">
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('municipalities'))) has-error @endif">
                        <input type="text" id="住所" name="municipalities" placeholder="〇〇市〇〇区〇〇町" value="{{ old('municipalities', $っｄ->municipalities) }}">
            ...
            <tr>
                <th>住所３（丁目番地）</th>
                <td colspan=3>
                    <input type="text" name="block" placeholder="〇〇-〇" value="{{ old('block', $っｄ->block) }}">
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('land_area'))) has-error @endif">
                        <input type="number" name="land_area" value="{{ old('land_area', $っｄ->land_area) }}">㎡
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('building_area'))) has-error @endif">
                        <input type="number" name="building_area" value="{{ old('building_area', $っｄ->building_area) }}">㎡
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('floor_area_ratio'))) has-error @endif">
                        <input type="number" name="floor_area_ratio" value="{{ old('floor_area_ratio', $っｄ->floor_area_ratio) }}">％
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('building_coverage_ratio'))) has-error @endif">
                        <input type="number" name="building_coverage_ratio" value="{{ old('building_coverage_ratio', $っｄ->building_coverage_ratio) }}">％
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('balcony_area'))) has-error @endif">
                        <input type="number" name="balcony_area" value="{{ old('balcony_area', $っｄ->balcony_area) }}">㎡
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('number_of_rooms'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('type_of_room'))) has-error @endif">
                            <input type="number" name="number_of_rooms" value="{{ old('number_of_rooms', $っｄ->number_of_rooms) }}">
                            <select name="type_of_room">
                                <option disabled selected>未選択</option>
                                <option value="K" @if(old('type_of_room', $っｄ->type_of_room) === 'K') selected @endif>K</option>
                                <option value="DK" @if(old('type_of_room', $っｄ->type_of_room) === 'DK') selected @endif>DK</option>
                                <option value="LDK" @if(old('type_of_room', $っｄ->type_of_room) === 'LDK') selected @endif>LDK</option>
                            </select>
                    ...
                    <div class="form-group @if(!empty($errors->first('year'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('month'))) has-error @endif">
                            <div class="form-group @if(!empty($errors->first('day'))) has-error @endif">
                                <input type="number" name="year" id="year" value="{{ old('year', $っｄ->year) }}" min="1" max="2100" step="1">年
                                <input type="number" name="month" id="month" value="{{ old('month', $っｄ->month) }}" min="1" max="12" step="1">月
                                <input type="number" name="day" id="day" value="{{ old('day', $っｄ->day) }}" min="1" max="31" step="1">日
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('building_construction'))) has-error @endif">
                        <input type="text" name="building_construction" value="{{ old('building_construction', $っｄ->building_construction) }}">
                ...
                <td>
                    <select name="parking_lot">
                        <option disabled selected>未選択</option>
                        <option value="なし" @if(old('parking_lot', $っｄ->parking_lot) === 'なし') selected @endif>なし</option>
                        <option value="１台" @if(old('parking_lot', $っｄ->parking_lot) === '１台') selected @endif>１台</option>
                        <option value="２台" @if(old('parking_lot', $っｄ->parking_lot) === '２台') selected @endif>２台</option>
                        <option value="３台" @if(old('parking_lot', $っｄ->parking_lot) === '３台') selected @endif>３台</option>
                        <option value="４台" @if(old('parking_lot', $っｄ->parking_lot) === '４台') selected @endif>４台</option>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('urban_planning'))) has-error @endif">
                        <select name="urban_planning">
                            ...
                            <option value="{{ $key }}" @if(old('urban_planning', $っｄ->urban_planning) === $urban_planning) selected @endif>{{ $urban_planning }}</option>
                            @endforeach
                        </select>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('land_use_zones'))) has-error @endif">
                        <select name="land_use_zones">
                            ...
                            <option value="{{ $key }}" @if(old('land_use_zones', $っｄ->land_use_zones) === $land_use_zones) selected @endif>{{ $land_use_zones }}</option>
                            @endforeach
                        </select>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('land_classification'))) has-error @endif">
                        <input type="text" name="land_classification" value="{{ old('land_classification', $っｄ->land_classification) }}">
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('land_right'))) has-error @endif">
                        <select name="land_right">
                            ...
                            <option value="{{ $key }}" @if(old('land_right', $っｄ->land_right) === $land_right) selected @endif>{{ $land_right }}</option>
                            @endforeach
                        </select>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('status'))) has-error @endif">
                        <select name="status">
                            <option disabled selected>未選択</option>
                            <option value="更地" @if(old('status', $っｄ->status) === '更地') selected @endif>更地</option>
                            <option value="建築中" @if(old('status', $っｄ->status) === '建築中') selected @endif>建築中</option>
                            <option value="完成済み" @if(old('status', $っｄ->status) === '完成済み') selected @endif>完成済み</option>
                        </select>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('other_fee'))) has-error @endif">
                        <input type="text" name="other_fee" value="{{ old('other_fee', $っｄ->other_fee) }}">
            ...
            <tr>
                <th>法令上の制限</th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="restrictions_by_law">{{ old('restrictions_by_law', $っｄ->restrictions_by_law) }}</textarea>
                </td>
                <th><label class="required">入居時期</label></th>
                <td>
                    <div class="form-group @if(!empty($errors->first('delivery_date'))) has-error @endif">
                        <input type="text" name="delivery_date" value="{{ old('delivery_date', $っｄ->delivery_date) }}">
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('terrain'))) has-error @endif">
                        <select name="terrain">
                            ...
                            <option value="{{ $key }}" @if(old('terrain', $っｄ->terrain) === $terrain) selected @endif>{{ $terrain }}</option>
                            @endforeach
                        </select>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('adjacent_road'))) has-error @endif">
                        <select name="adjacent_road">
                            ...
                            <option value="{{ $key }}" @if(old('adjacent_road', $っｄ->adjacent_road) === $adjacent_road) selected @endif>{{ $adjacent_road }}</option>
                            @endforeach
                        </select>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('adjacent_road_width'))) has-error @endif">
                        <input type="text" name="adjacent_road_width" value="{{ old('adjacent_road_width', $っｄ->adjacent_road_width) }}">ｍ
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('private_road'))) has-error @endif">
                        <select name="private_road">
                            <option disabled selected>未選択</option>
                            <option value="あり" @if(old('private_road', $っｄ->private_road) === 'あり') selected @endif>あり</option>
                            <option value="なし" @if(old('private_road', $っｄ->private_road) === 'なし') selected @endif>なし</option>
                        </select>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('setback'))) has-error @endif">
                        <input type="text" name="setback" value="{{ old('setback', $っｄ->setback) }}" placeholder="">
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('water_supply'))) has-error @endif">
                        <select name="water_supply">
                            ...
                            <option value="{{ $key }}" @if(old('water_supply', $っｄ->water_supply) === $water_supply) selected @endif>{{ $water_supply }}</option>
                            @endforeach
                        </select>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('sewage_line'))) has-error @endif">
                        <select name="sewage_line">
                            ...
                            <option value="{{ $key }}" @if(old('sewage_line', $っｄ->sewage_line) === $sewage_line) selected @endif>{{ $sewage_line }}</option>
                            @endforeach
                        </select>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('gas'))) has-error @endif">
                        <select name="gas">
                            ...
                            <option value="{{ $key }}" @if(old('gas', $っｄ->gas) === $gas) selected @endif>{{ $gas }}</option>
                            @endforeach
                        </select>
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('building_certification_number'))) has-error @endif">
                        <input type="text" name="building_certification_number" value="{{ old('building_certification_number', $っｄ->building_certification_number) }}">
            ...
            <tr>
                ...
                <td>
                    <input type="number" name="walking_distance_station" value="{{ old('walking_distance_station', $っｄ->walking_distance_station) }}">分
                </td>
            </tr>
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('conditions_of_transactions'))) has-error @endif">
                        <select name="conditions_of_transactions">
                            ...
                            <option value="{{ $key }}" @if(old('conditions_of_transactions', $っｄ->onditions_of_transactions) === $conditions_of_transactions) selected @endif>{{ $conditions_of_transactions }}</option>
                            @endforeach
                        </select>
            ...
            <tr>
                <th>小学校</th>
                <td colspan=3>
                    <input type="text" placeholder="〇〇小学校" name="elementary_school_name" value="{{ old('elementary_school_name', $っｄ->elementary_school_name) }}">まで徒歩</th><input type="number" name="elementary_school_district" value="{{ old('elementary_school_district', $っｄ->elementary_school_district) }}">分
                </td>
            </tr>
            <tr>
                <th>中学校</th>
                <td colspan=3>
                    <input type="text" placeholder="〇〇中学校" name="junior_high_school_name" value="{{ old('junior_high_school_name', $っｄ->junior_high_school_name) }}">まで徒歩</th><input type="number" name="junior_high_school_district" value="{{ old('junior_high_school_district', $っｄ->junior_high_school_district) }}">分
                </td>
            </tr>
                <th>物件画像（複数可能）</th>
                <td colspan=3>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <th><p>セールスコメント</p></th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="sales_comment">{{ old('sales_comment', $っｄ->sales_comment) }}</textarea>
                </td>
            </tr>
            <tr>
                <th><p>物件紹介コメント</p></th>
                <td>
                    <textarea maxlength="800" placeholder="最大800文字まで" name="property_introduction">{{ old('property_introduction', $っｄ->property_introduction) }}</textarea>
                </td>
            </tr>
            <tr>
                <th><p>設備条件</p></th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="terms_and_conditions">{{ old('terms_and_conditions', $っｄ->terms_and_conditions) }}</textarea>
                </td>
            </tr>
        </tbody>
    </table>
