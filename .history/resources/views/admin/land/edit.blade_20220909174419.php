    ...
    <table>
        ...
        <tbody>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('price'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('tax'))) has-error @endif">
                            <input type="number" step="0.1"min="0.1" max="99999.9" name="price" value="{{ old('price', $land->price) }}"> 万円
                            <select name="tax">
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('pref'))) has-error @endif">
                        <input type="text" maxlength="4" id="都道府県" name="pref" placeholder="〇〇県" value="{{ old('pref', $land->pref) }}">
            ...
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('municipalities'))) has-error @endif">
                        <input type="text" maxlength="70" id="住所" name="municipalities" placeholder="〇〇市〇〇区〇〇町" value="{{ old('municipalities', $land->municipalities) }}">
            ...
            <tr>
                <th>住所３（丁目番地）</th>
                <td colspan=3>
                    <input type="text" maxlength="30" name="block" placeholder="〇〇-〇" value="{{ old('block', $land->block) }}">
                </td>
            </tr>
            <tr>
                ...
                <td colspan=3>
                    <div class="form-group @if(!empty($errors->first('land_area'))) has-error @endif">
                        <input type="number" step="0.01" min="0.01" max="99999.99" name="land_area" value="{{ old('land_area', $land->land_area) }}"> ㎡
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('floor_area_ratio'))) has-error @endif">
                        <input type="number" min="50" max="1300" name="floor_area_ratio" value="{{ old('floor_area_ratio', $land->floor_area_ratio) }}"> ％
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('building_coverage_ratio'))) has-error @endif">
                        <input type="number" min="30" max="80" name="building_coverage_ratio" value="{{ old('building_coverage_ratio', $land->building_coverage_ratio) }}"> ％
                        ...
                        <div class="form-group @if(!empty($errors->first('adjacent_road'))) has-error @endif">
                            <div class="form-group @if(!empty($errors->first('adjacent_road_width'))) has-error @endif">
                                <select name="adjacent_road_direction">
                                    ...
                                    <option value="{{ $key }}" @if(old('adjacent_road_direction', $land->adjacent_road_direction) === $adjacent_road_direction) selected @endif>{{ $adjacent_road_direction }}</option>
                                    @endforeach
                                </select>
                                ...
                                <input type="text" name="adjacent_road_width" value="{{ old('adjacent_road_width', $land->adjacent_road_width) }}" placeholder="接道幅"> ｍ
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('setback'))) has-error @endif">
                        <div class="form-group @if(!empty($errors->first('setback_length'))) has-error @endif">
                            <input type="radio" name="setback" id="noSetback" value="なし" onclick="setbackEdit()" @if(old('setback', $land->setback) === 'なし') checked @endif><label for="noSetback">なし</label>
                            <input type="radio" name="setback" id="haveSetback" value="あり" onclick="setbackEdit()" @if(old('setback', $land->setback) === 'あり') checked @endif><label for="haveSetback">あり</label>
                            <div id="setbackLengthForm">
                                <label for="setbackLength">セットバック幅：<input type="number" step="0.01" min="0.01" max="4.00" name="setback_length" id="setbackLength" value="{{ old('setback_length', $land->setback_length) }}"> m</label>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="form-group @if(!empty($errors->first('station'))) has-error @endif">
                            <select name="station" id="station">
                                <option value="{{ $land->station }}" @if(old('station', $land->station) === $land->station) selected @endif>{{ $land->station }}</option>
                            </select>
                    ...
                    <td>
                        <div class="form-group @if(!empty($errors->first('access_method'))) has-error @endif">
                            <select name="access_method">
                                ...
                                <option value="{{ $key }}" @if(old('access_method', $land->access_method) === $access_method) selected @endif>{{ $access_method }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @if(!empty($errors->first('distance_station'))) has-error @endif">
                            <input type="number" step="1" min="1" max="60" name="distance_station" value="{{ old('distance_station', $land->distance_station) }}"> 分
            ...
            <tr>
                <th>小学校</th>
                <td colspan=3>
                    <input type="text" placeholder="〇〇小学校" name="elementary_school_name" value="{{ old('elementary_school_name', $land->elementary_school_name) }}"> まで徒歩 </th><input type="number" step="1" min="1" max="60" name="elementary_school_district" value="{{ old('elementary_school_district', $land->elementary_school_district) }}"> 分
                </td>
            </tr>
            <tr>
                <th>中学校</th>
                <td colspan=3>
                    <input type="text" placeholder="〇〇中学校" name="junior_high_school_name" value="{{ old('junior_high_school_name', $land->junior_high_school_name) }}"> まで徒歩 </th><input type="number" step="1" min="1" max="60" name="junior_high_school_district" value="{{ old('junior_high_school_district', $land->junior_high_school_district) }}"> 分
                </td>
            </tr>
        </tbody>
    </table><br>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <th>セールスコメント</th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="sales_comment" id="countSalesComment">{{ old('sales_comment', $land->sales_comment) }}</textarea>
            ...
            <tr>
                <th>物件紹介コメント</th>
                <td>
                    <textarea maxlength="800" placeholder="最大800文字まで" name="property_introduction" id="countIntroComment">{{ old('property_introduction', $land->property_introduction) }}</textarea>
            ...
            <tr>
                <th>設備条件</th>
                <td>
                    <textarea maxlength="200" placeholder="最大200文字まで" name="terms_and_conditions" id="countTerms">{{ old('terms_and_conditions', $land->terms_and_conditions) }}</textarea>
