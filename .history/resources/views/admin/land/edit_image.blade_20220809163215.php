...
<table class="edit_menu">
    <tbody>
        <td><a href="{{ route('admin..edit', ['id' => $->id]) }}" class="edit_menu_link">物件概要</a></td>
        <td><a href="{{ route('admin.Image.edit', ['id' => $->id]) }}" class="edit_menu_link">物件画像</a></td>
    </tbody>
</table><br>
        ...
        <tr class="property_images">
            @foreach($_images as $_image)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.Image.update', ['id' => $_image->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
                        <div id="signUpForm{{ $image_counter }}">
                            <img src="{{ asset('/storage/property_images//' . $_image->path) }}" id="showImage{{ $image_counter }}" class="show_image" alt="物件画像{{ $image_counter }}"><br>
                            <input type="hidden" id="imageCounter" value="{{ $image_counter }}">
                            <input type="button" id="ImageDelete" onclick="deletePropertyImage({{ $image_counter }})" value="画像削除">
                        </div>
                        ...
                        <div class="form-group @if(!empty($errors->first( "category" . $image_counter))) has-error @endif">
                            <select name="category">
                                ...
                                @foreach (PropertyInformationConsts::_IMAGES_CATEGORY_LIST as $key => $category)
                                <option value="{{ $key }}" @if(old('category', $_image->category) === $category) selected @endif>{{ $category }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @if(!empty($errors->first( "comment" . $image_counter ))) has-error @endif">
                            <textarea name="comment" cols="150" rows="5">{{ old('comment' . $image_counter, $_image->comment) }}</textarea>
                            <span class="help-block">{{$errors->first( "comment" . $image_counter )}}</span>
                        </div>

                        <input type="submit" id="ImageUpdate" value="登録">
                    </form>
                    <form action="{{ route('admin.Image.delete', ['id' => $_image->id]) }}">
            ...
            @while($image_counter < 21)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.Image.signUp', ['id' => $->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
