...
<table class="edit_menu">
    <tbody>
        <td><a href="{{ route('admin.newDetachedHouseGroup.edit', ['id' => $new_detached_house_grouo_group->id]) }}" class="edit_menu_link">物件概要</a></td>
        <td><a href="{{ route('admin.newDetachedHouseGroupImage.edit', ['id' => $new_detached_house_grouo_group->id]) }}" class="edit_menu_link">物件画像</a></td>
    </tbody>
</table><br>
...
<table class="photo_edit_table">
    ...
    <tbody>
        <tr class="property_images">
            @foreach($new_detached_house_grouo_images as $new_detached_house_grouo_image)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.newDetachedHouseGroupImage.update', ['id' => $new_detached_house_grouo_image->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
                        <div id="signUpForm{{ $image_counter }}">
                            <img src="{{ asset('/storage/property_images/new_detached_house_grouo/' . $new_detached_house_grouo_image->path) }}" id="showImage{{ $image_counter }}" class="show_image" alt="物件画像{{ $image_counter }}"><br>
                        ...
                        <div class="form-group @if(!empty($errors->first( "category" . $image_counter))) has-error @endif">
                            <select name="category">
                                ...
                                <option value="{{ $key }}" @if(old('category', $new_detached_house_grouo_image->category) === $category) selected @endif>{{ $category }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @if(!empty($errors->first( "comment" . $image_counter ))) has-error @endif">
                            <textarea name="comment" cols="150" rows="5">{{ old('comment' . $image_counter, $new_detached_house_grouo_image->comment) }}</textarea>
                    ...
                    <form action="{{ route('admin.newDetachedHouseGroupImage.delete', ['id' => $new_detached_house_grouo_image->id]) }}">
            ...
            @while($image_counter < 21)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.newDetachedHouseGroupImage.signUp', ['id' => $new_detached_house_grouo->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
