...
<table>
    ...
    <tbody>
        ...
        <tr>
            <th>完成時期（築年月）</th>
            <td>
                @if(!empty($new_detached_house_group->day))
                    {{ $new_detached_house_group->year }}年{{ $new_detached_house_group->month }}月{{ $new_detached_house_group->day }}日
                @elseif(empty($new_detached_house_group->day))
                    {{ $new_detached_house_group->year }}年{{ $new_detached_house_group->month }}月
                @endif
            </td>
            <th>建物構造</th>
            <td>
        ...
        <tr>
            <th>最寄り駅</th>
            <td colspan=3>
                {{ $new_detached_house_group->station }}まで{{ $new_detached_house_group->access_method }}{{ $new_detached_house_group->distance_station }}分
            </td>
        </tr>
    </tbody>
</table><br>
