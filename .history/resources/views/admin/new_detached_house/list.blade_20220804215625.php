    ...
    <tbody>
        @forelse($new_detached_houses as $new_detached_house)
            <tr>
                ...
                <td>
                    <form methid="POST" action="{{ route('admin.new_detached_houseRecommend.signUp', ['id' => $new_detached_house->id]) }}">
                        @csrf
                        <input type="submit" id="new_detached_houseRecommend" value="おすすめ登録">
                    </form>
                </td>
                <td><button onclick="location.href='{{ route('admin.new_detached_house.detail', ['id' => $new_detached_house->id]) }}'">詳細</button></td>
                <td><button id="new_detached_houseDelete">削除</button></td>
            </tr>
        @empty
