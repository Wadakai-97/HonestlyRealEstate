...
@section('body')
<h2>{{ $new_detached_house->name }}：詳細</h2>
<table>
    <tbody>
        <tr>
        ...
        <tr>
            ...
            <th>駐車場</th>
            <td>
                {{ $new_detached_house->parking_lot }}
            </td>
        </tr>
        <tr>
            <th>用途地域</th>
        ...
        <tr>
            <th>最寄り駅</th>
            <td>
                {{ $new_detached_house->station->station_name }}まで徒歩{{ $new_detached_house->walking_distance_station }}分
            </td>
            <th>取引態様</th>
            <td>
...
<form action="{{ route('admin.newDetachedHouse.edit', ['id' => $new_detached_house->id]) }}">
    @csrf
