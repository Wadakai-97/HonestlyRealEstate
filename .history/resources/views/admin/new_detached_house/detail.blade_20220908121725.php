...
<table>
    ...
    <tbody>
        ...
        <tr>
            <th>完成時期（築年月）</th>
            <td>
                @if(!empty($new_detached_house->day))
                    {{ $new_detached_house->year }}年{{ $new_detached_house->month }}月{{ $new_detached_house->day }}日
                @elseif(empty($new_detached_house->day))
                    {{ $new_detached_house->year }}年{{ $new_detached_house->month }}月
                @endif
            </td>
            <th>建物構造</th>
            <td>
        ...
        <tr>
            ...
            <th>駐車場</th>
            <td>
                @if($new_detached_house->parking == 'なし' || $new_detached_house->parking_lot == '0')
                    なし
                @elseif($new_detached_house->parking == 'あり' )
                    {{ $new_detached_house->parking_lot }}台
                @endif
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <th>最寄り駅</th>
            <td>
                {{ $new_detached_house->station }}まで{{ $new_detached_house->access_method }}{{ $new_detached_house->distance_station }}分
            </td>
            <th>取引態様</th>
            <td>
