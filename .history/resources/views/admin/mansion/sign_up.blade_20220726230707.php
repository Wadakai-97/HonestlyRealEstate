            ...
            <tr class="property_images">
                @for($i = 1; $i < 21; $i++)
                    <td class="property_images_block">
                        ...
                        <input type="file" name="image{{ $i }}" id="inputImage{{ $i }}" onchange="previewPropertyImage(event, {{ $i }})"><br>
                        ...
                        <p>コメント</p>
                        <div class="form-group @if(!empty($errors->first('' {{ $i }} ))) has-error @endif">
                            <textarea name="comment{{ $i }}" cols="150" rows="5"></textarea>
                            <span class="help-block">{{$errors->first('' {{ $i }} )}}</span>
                        </div><br>
                    </td>
                @endfor
            </tr>
