...
<form method="post" action="{{ route('admin.mansion.update', ['id' => $mansion->id]) }}" enctype="multipart/form-data">
    <table>
        ...
        <tbody>
            <tr>
                <td colspan=2><input type="file" name="files[][image]" multiple></td>
            </tr>
            <tr>
                @foreach ($mansion_images as $mansion_image)
                <td>
                    <form method="post" action="{{ route('admin.mansion.update_photo', ['id' => $mansion->id]) }}" enctype="multipart/form-data">
