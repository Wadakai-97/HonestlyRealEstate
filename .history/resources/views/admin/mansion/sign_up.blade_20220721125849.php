...
<form method="post" action="{{ route('admin.mansion.signUp', ['id' => $i]) }}" enctype="multipart/form-data">
    @csrf
    @method('patch')
    <table>
        <tbody>
            ...
            <tr class="property_images">
                @for($i = 1; $i < 21; $i++)
                    <td class="property_images_block">
                        ...
                        <div id="{{ $i }}"></div>
                        <input type="file" id="inputImage{{ $i }}" onchange="previewPropertyImage(event, {{ $i }})"><br>
