            ...
            @while ($image_counter < 21)
                @foreach($mansion_images as $mansion_image)
                    <td class="property_images_block">
                        <form method="post" action="{{ route('admin.mansionImage.updateImage', ['id' => $mansion_image->id]) }}" enctype="multipart/form-data">
                            @csrf
                            @method('patch')
                ...
                @while($image_counter < 21)
                    <td class="property_images_block">
                        <form method="post" action="{{ route('admin.mansion.signUp', ['id' => $mansion->id]) }}" enctype="multipart/form-data">
                            @csrf
                            @method('patch')
                            ...
                            <div class="form-group @if(!empty($errors->first("image" . $image_counter))) has-error @endif">
                                <input type="file" name="image{{ $image_counter }}" id="inputImage{{ $image_counter }}" onchange="previewPropertyImage(event, {{ $image_counter }})">
                            ...
                            <div class="form-group @if(!empty($errors->first( "comment" . $image_counter ))) has-error @endif">
                                <textarea name="comment{{ $image_counter }}" cols="150" rows="5">{{ old('comment' . $image_counter) }}</textarea>
