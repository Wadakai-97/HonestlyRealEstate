...
<table class="photo_edit_table">
    ...
    <tbody>
        <tr class="property_images">
            @foreach($old_detached_house_images as $old_detached_house_image)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.oldDetachedHouseImage.update', ['id' => $old_detached_house_image->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
                        <div id="signUpForm{{ $image_counter }}">
                            <img src="{{ asset('/storage/property_images/old_detached_house/' . $old_detached_house_image->path) }}" id="showImage{{ $image_counter }}" class="show_image" alt="物件画像{{ $image_counter }}"><br>
                            <input type="hidden" id="imageCounter" value="{{ $image_counter }}">
                            <input type="button" id="oldDetachedHouseImageDelete" onclick="deletePropertyImage({{ $image_counter }})" value="画像削除">
                        </div>
                        ...
                        <div class="form-group @if(!empty($errors->first( "category" . $image_counter))) has-error @endif">
                            <select name="category">
                                ...
                                <option value="{{ $key }}" @if(old('category', $old_detached_house_image->category) === $category) selected @endif>{{ $category }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @if(!empty($errors->first( "comment" . $image_counter ))) has-error @endif">
                            <textarea name="comment" cols="150" rows="5">{{ old('comment' . $image_counter, $old_detached_house_image->comment) }}</textarea>
                            <span class="help-block">{{$errors->first( "comment" . $image_counter )}}</span>
                        </div>

                        <input type="submit" id="oldDetachedHouseImageUpdate" value="登録">
                    </form>
                    <form action="{{ route('admin.oldDetachedHouseImage.delete', ['id' => $old_detached_house_image->id]) }}">
            ...
            @while($image_counter < 21)
                <td class="property_images_block">
                    <form method="post" action="{{ route('admin.oldDetachedHouseImage.signUp', ['id' => $old_detached_house->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <p>画像{{ $image_counter }}</p>
