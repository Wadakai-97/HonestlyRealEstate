    ...
    <tbody>
        @forelse($old_detached_houses as $old_detached_house)
            <tr>
                ...
                <td>{{ $old_detached_house->price }}</td>
                ...
                <td>
                    <form methid="POST" action="{{ route('admin.oldDetachedRecommend.signUp', ['id' => $old_detached_house->id]) }}">
                        @csrf
                        <input type="submit" id="oldDetachedRecommend" value="おすすめ登録">
                    </form>
                </td>
                <td><button onclick="location.href='{{ route('admin.old_detached_house.detail', ['id' => $old_detached_house->id]) }}'">詳細</button></td>
                <td><button id="oldDetachedDelete">削除</button></td>
            </tr>
        @empty
