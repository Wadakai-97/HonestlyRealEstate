...
<form action="{{ route('land.sort') }}" method="post">
    @csrf
    <select name="sort" onchange="submit(this.form)">
        <option disabled selected>-- 未選択 --</option>
        @foreach (FilteringConsts::LAND_SORT_LIST as $key => $sort)
    ...
    @forelse ($lands as $land)
        <tbody>
            <tr>
                ...
                <td>{{ $land->station }}まで{{ $land->access_method }}{{ $land->distance_station }}分</td>
                <td>
                    <form action="{{ route('favorite.signUp',  ['id' => $land->id]) }}" method="post">
                        @csrf
...
<div class="d-flex justify-content-center">
    {{ $lands->appends(request()->input())->links('pagination::default') }}
</div>
@endsection
