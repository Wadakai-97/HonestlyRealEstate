...
<form action="{{ route('newDetachedHouse.sort') }}" method="post">
    @csrf
    <select name="sort" onchange="submit(this.form)">
        <option disabled selected>-- 未選択 --</option>
        @foreach (FilteringConsts::DETACHED_HOUSE_SORT_LIST as $key => $sort)
    ...
    <input type="hidden" name="land_right" value="{{ $request->land_right }}">
    <input type="hidden" name="lowest_land_area" value="{{ $request->lowest_land_area }}">
    <input type="hidden" name="highest_land_area" value="{{ $request->highest_land_area }}">
    <input type="hidden" name="lowest_building_area" value="{{ $request->lowest_building_area }}">
    <input type="hidden" name="highest_building_area" value="{{ $request->highest_building_area }}">
    ...
    <div class="search_condition">
        ...
        <p>
            ...
            @if(!empty(request()->lowest_land_area) && empty(request()->highest_land_area))
                【最低土地面積】{{ number_format($request->lowest_land_area) }}㎡
            @endif
            @if(empty(request()->lowest_land_area) && !empty(request()->highest_land_area))
                【最高土地面積】{{ number_format($request->highest_land_area) }}㎡
            @endif
            @if(!empty(request()->lowest_land_area) && !empty(request()->highest_land_area))
                【最低土地面積〜最高土地面積】{{ number_format($request->lowest_land_area) }}㎡〜{{ number_format($request->highest_land_area) }}㎡
            @endif
...
<table class="result_table">
    ...
    @forelse ($new_detached_houses as $new_detached_house)
        <tbody>
            <tr>
                ...
                <td>{{ number_format($new_detached_house->land_area) }}㎡</td>
                <td>{{ number_format($new_detached_house->building_area) }}㎡</td>
                <td>{{ $new_detached_house->land_right }}</td>
                <td>{{ $new_detached_house->station }}まで{{ $new_detached_house->distance_station }}分</td>
    ...
    @forelse ($new_detached_house_groups as $new_detached_house_group)
        <tbody>
            <tr>
                <td><a href="{{ route('newDetachedHouse.detail', ['id' => $new_detached_house_group->id]) }}" class="font_link">{{ $new_detached_house_group->name }}</a></td>
                <td>{{ $new_detached_house_group->pref }}{{ $new_detached_house_group->municipalities }}</td>
                ...
                <td>{{ number_format($new_detached_house_group->lowest_land_area) }}㎡〜{{ $new_detached_house_group->highest_land_area }}</td>
                <td>{{ number_format($new_detached_house_group->lowest_building_area) }}㎡〜{{ $new_detached_house_group->highest_building_area }}</td>
                ...
                <td>{{ $new_detached_house_group->updated_at->format('Y年m月d日') }}</td>
            </tr>
        </tbody>
    @empty
        <p>検索条件に一致する物件はありませんでした。</p>
