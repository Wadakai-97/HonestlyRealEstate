...
<form action="{{ route('mansion.sort') }}" method="post">
    @csrf
    <select name="column" onchange="submit(this.form)">
        <option disabled selected>-- 未選択 --</option>
        @foreach (FilteringConsts::MANSION_SPRT_LIST as $key => $sort)
        <option value="{{ $key }}" @if(old('sort') === $sort) selected @endif>{{ $sort }}万円以上</option>
        @endforeach
