...
<form class="loan_payment_simulation">
    <table>
        <tbody>
            <tr>
                <th>物件価格</th>
                <td colspan=4><input type="hidden" id="price" value="{{ $mansion->price }}">{{ $mansion->price }}万円（{{ $mansion->tax }}）</td>
            </tr>
            <tr>
                <th>金利</th>
                <td><label><input type="radio" value="0.475" id="annual_interest">変動金利：0.475%</label></td>
                <td><label><input type="radio" value="1.35" id="annual_interest">固定金利：1.35%</label></td>
                <td colspan="2"><input type="radio" id="annual_interest"><input id="annual_interest" type="text" value="">%</td>
            </tr>
            <tr>
                <th>借入期間</th>
                <td><label><input type="radio" value="35" id="borrowing_period" onclick="interestText()">35年</label></td>
                <td><label><input type="radio" value="30" id="borrowing_period" onclick="interestText()">30年</label></td>
                <td><label><input type="radio" value="25" id="borrowing_period" onclick="interestText()">25年</label></td>
                <td><input type="radio" id="borrowing_period" class="other_interest" onclick="interestText()"><div class="interest_text"><input type="text" value="" id="borrowing_period">年</div></td>
            </tr>
            <tr>
                <th>頭金</th>
                <td colspan="2"><label><input type="radio" value="0" id="down_payment">なし</label></td>
                <td colspan="2"><label><input type="radio" id="down_payment"><input id="down_payment" type="text" value=""> 万円</label></td>
            </tr>
            <tr>
                <th>
    ...
    <form method="POST" action="{{ route('contact.confirm') }}" class="contact_form">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <label name="contents"><input type="checkbox" value="実際に物件を見たい" name="contents[]">実際に物件を見たい</label></td>
                    <label name="contents"><input type="checkbox" value="詳細な資料が欲しい" name="contents[]">詳細な資料がほしい</label></td>
                    <label name="contents"><input type="checkbox" value="住宅購入に関して相談したい" name="contents[]">住宅購入に関して相談したい</label></td>
                    <label name="contents"><input type="checkbox" value="希望条件に沿う物件を見学したい" name="contents[]">類似物件も見たい</label></td>
            </tr>
            <tr>
                <th><label class="required">お問い合わせ内容</label></th>
                <td colspan="4"><textarea name="customer_request" placeholder="今週の土曜日１０時に５名で購入相談と物件見学を希望します。希望条件は〇〇のため、事前にご用意いただけますと幸いです。"></textarea></td>
            </tr>
        </table>
