...
<h3 class="secondary_item">建物概要</h3>
    <table>
        ...
        <tbody>
            <tr>
                <th>物件名</th>
                    <td colspan=3>{{ $mansion->apartment_name }}</td>
            </tr>
            <tr>
                <th>価格</th>
                    <td colspan=3>{{ number_format($mansion->price) }}万円</td>
            </tr>
            <tr>
                <th>所在地</th>
                    <td colspan=3>{{ $mansion->pref }}{{ $mansion->municipalities }}{{ $mansion->block }}</td>
            </tr>
            <tr>
                <th>最寄り駅</th>
                    <td colspan=3>{{ $mansion->station }}まで{{ $mansion->access_method }}{{ $mansion->distance_station }}分</td>
            </tr>
            <tr>
                <th>間取り</th>
            ...
            <tr>
                <th>所在階/建物構造</th>
                    <td colspan=3>{{ $mansion->floor }}階/{{ $mansion->building_construction }}{{ $mansion->story }}</td>
            </tr>
            <tr>
                <th>現況</th>
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('phone'))) has-error @endif">
                        <input name="phone" type="text" value="{{ old('phone') }}" placeholder="例）000-0000-0000">
            ...
            <tr>
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('address'))) has-error @endif">
                        <input name="address" type="text" value="{{ old('address') }}" placeholder="例）〇〇県〇〇市〇〇町〇-〇">
            ...
            <tr class="small_textarea">
                ...
                <td>
                    <div class="form-group @if(!empty($errors->first('messages'))) has-error @endif">
                        <textarea value="{{ old('messages') }}" placeholder="例）今週の土曜日１０時に５名で購入相談と物件見学を希望します。"></textarea>
