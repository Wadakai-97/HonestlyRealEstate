...
@if(!empty(request()))
    <div class="search_condition">
        ...
        @if(!empty(request()->lowest_price) && empty(request()->highest_price))
            最低価格：{{ $request->lowest_price }}万円</p>
        @endif
        @if(empty(request()->lowest_price) && !empty(request()->highest_price))
            最高価格：{{ $request->highest_price }}万円</p>
        @endif
        @if(!empty(request()->lowest_price) && !empty(request()->highest_price))
            最低価格：{{ $request->lowest_price }}万円〜最高価格：{{ $request->highest_price }}万円</p>
        @endif

        {{-- 専有面積 --}}
        @if(!empty(request()->lowest_occupation_area) && empty(request()->highest_occupation_area))
            最低専有面積：{{ $request->lowest_occupation_area }}㎡</p>
        @endif
        @if(empty(request()->lowest_occupation_area) && !empty(request()->highest_occupation_area))
            最高専有面積：{{ $request->highest_occupation_area }}㎡</p>
        @endif
        @if(!empty(request()->lowest_occupation_area) && !empty(request()->highest_occupation_area))
            最低専有面積：{{ $request->lowest_occupation_area }}㎡〜最高専有面積：{{ $request->highest_occupation_area }}㎡</p>
        @endif
        ...
        @if(!empty(request()->old))
            築年数：{{ $request->old }}年以内</p>
        @endif

        {{-- 最寄り駅 --}}
        @if(!empty(request()->station))
            最寄り駅：{{ $request->station }}</p>
        @endif

        {{-- 駅徒歩 --}}
        @if(!empty(request()->walking_distance_station))
            駅徒歩：{{ $request->walking_distance_station }}分</p>
        @endif
    </div>
@endif
