...
$sql = 'SELECT id, apartment_name, number_of_rooms, type_of_room, price, municipalities FROM mansions WHERE 1 ';
...
if (!empty($_POST['apartment_name_keyword'])) {
    $sql .= ' AND apartment_name LIKE :apartment_name_keyword ';
}
...
if (!empty($_POST['municipalities_keyword'])) {
    $sql .= ' AND municipalities LIKE :municipalities_keyword ';
}

$stmt = $dbh->prepare($sql);

if (!empty($apartment_name_keyword)) {
    $stmt->bindValue(':apartmentt_name_keyword', "%".$apartment_name_keyword."%", PDO::PARAM_STR);
}
