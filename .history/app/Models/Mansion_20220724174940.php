...
class Mansion extends Model
{
    ...
    // Scope
    public function scopeWherePref($mansion, $request) {
        $pref = $request->pref;
        if(!empty($pref)) {
            $mansion->Where(function ($request) use($pref) {
                for ($i = 0; $i < count($pref); $i++){
                    $mansion->where('pref', '=',  $pref[$i]);
                }
            });
        }
    }
    public function scopeWhereLowestPrice($mansion, $request) {
        $lowest_price = $request->lowest_price;
        if(!empty($lowest_price)) {
            $mansion->where('price', '>=', $lowest_price);
        }
    }
    public function scopeWhereHighestPrice($mansion, $request) {
        $highest_price = $request->highest_price;
        if(!empty($highest_price)) {
            $mansion->where('price', '<=', $highest_price);
        }
    }
    public function scopeWhereLowestOccupationArea($mansion, $request) {
        $lowest_occupation_area = $request->lowest_occupation_area;
        if(!empty($lowest_occupation_area)) {
            $mansion->where('occupation_area', '>=', $lowest_occupation_area);
        }
    }
    public function scopeWhereHighestOccupationArea($mansion, $request) {
        $highest_occupation_area = $request->highest_occupation_area;
        if(!empty($highest_occupation_area)) {
            $mansion->where('occupation_area', '<=', $highest_occupation_area);
        }
    }
    public function scopeWherePlan($mansion, $request) {
        $plan = $request->plan;
        if(!empty($plan)) {
            $mansion->Where(function ($request) use($plan) {
                for ($i = 0; $i < count($plan); $i++){
                    $mansion->where('plan', '=',  $plan[$i]);
                }
            });
        }
    }
    public function scopeWhereOld($mansion, $request) {
        $years_ago = $request->old;
        if(!empty($years_ago)) {
            $old = Carbon::today()->subYear($years_ago);
            $mansion->where('year', '<=', $old);
        }
    }
    public function scopeWhereStation($mansion, $request) {
        $station = $request->station;
        if(!empty($station)) {
            $mansion->where('station', '=', $station);
        }
    }
    public function scopeWhereWalkingDistanceStation($mansion, $request) {
        $walking_distance_station = $request->walking_distance_station;
        if(!empty($walking_distance_station)) {
            $mansion->where('walking_distance_station', '<=', $walking_distance_station);
        }
    }
