...
class LandImage extends Model
{
    ...
    public function imageSignUp($id, $request) {
        DB::transaction(function() use ($id, $request) {
            ...
            if(!is_array($land_images && !empty($land_images))) {
                $image_id = 1;
            } else if(is_array($land_images) && !empty($land_images)) {
                $id_checker = range(1, 21);
                $not_dupulicate_number = array_diff($id_checker, $land_images);
            ...
            if(!empty($request->file('image'))) {
                ...
                $request->file('image')->storeAs('/storage/property_images/land_images', $file_name);
    ...
    public function imageUpdate($id, $request) {
        DB::transaction(function() use ($id, $request) {
            ...
            if(!empty($request->file('image')) && $land_image->path != $request->file('image')) {
                Storage::delete('/storage/land_images/' . $land_image->path);
                $extension = $request->file('image')->guessExtension();
                $file_name = "No{$land_image->land_id}_{$land_image->image_id}.{$extension}";
                $request->file('image')->storeAs('/storage/land_images', $file_name);
                $land_image->fill([
                    'path' => $file_name,
                    'category' => $request->category,
                    'comment' => $request->comment,
