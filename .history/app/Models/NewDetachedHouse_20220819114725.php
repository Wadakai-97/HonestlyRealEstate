...
class NewDetachedHouse extends Model
{
    ...
    public function sort($request) {
        if($request->sort == "新着順" || $request->sort == "価格が高い順" || $request->sort == "土地面積が広い順"|| $request->sort == "土地面積が広い順" || $request->sort == "間取りが広い順" || $request->sort == "築年数が新しい順") {
            $type = 'desc';
        }
        if($request->sort == "価格が安い順" || $request->sort == "土地面積が狭い順" || $request->sort == "間取りが狭い順" || $request->sort == "築年数が古い順") {
            $type = 'asc';
        }
        return $type;
    }
