...
class Mansion extends Model
{
    ...
    public function scopeWhereNumberOfRooms($query, $request) {
        $number_of_rooms = $request->number_of_rooms;
        if(!empty($number_of_rooms)) {
            $query->Where(function ($request) use($number_of_rooms) {
                for ($i = 0; $i < count($number_of_rooms); $i++){
                    $query->where('number_of_rooms', '=',  $number_of_rooms[$i]);
                }
            });
        }
    }
