...
class Mansion extends Model
{
    ...
    public function scopeWhereHighestOccupationArea($query, $highest_occupation_area) {
    ...
    public function scopeWhereOld($query, $old) {
        if(!empty($old)) {
            $years_old = Carbon::today()->year - $old;
            // $this_year = Carbon::today()->year - $old;
            // $years_ago = $this_year - $old;
            $query->where('year', '>=', (int)$years_ago);
        }
    }
