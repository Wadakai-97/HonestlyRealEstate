...
class NewDetachedHouse extends Model
{
    ...
    public function search() {
        ...
        if(!empty($number_of_rooms)) {
            $query = $query->Where(function ($query) use($num) {
                for ($i = 0; $i < count($num); $i++){
                    $query->orwhere('num', '=',  $num[$i]);
                }
            });
        }
        if(!empty($lowest_land_area)) {
            $query = $query->where('land_area', '>', $lowest_land_area);
        }
        if(!empty($highest_land_area)) {
            $query = $query->where('land_area', '<', $highest_land_area);
        }
    }
