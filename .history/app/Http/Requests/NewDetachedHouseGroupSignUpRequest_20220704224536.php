    ...
    public function rules()
    {
        return [
            ...
            'highest_building_coverage_ratio' => ['required', 'integer', 'regex:/^[!-~]+$/', 'digits_between:2,3',
                function($attribute, $value, $fail) {
                    $request = $this->all();
                    if (!empty($request['lowest_building_coverage_ratio']) && !empty($request['highest_building_area'])) {
                        if(($request['highest_building_area'] - $request['lowest_building_coverage_ratio']) < 0) {
                            $fail('最高建物面積は最低建物面積よりも大きい金額を入力してください。');
                        }
                    }
                }
