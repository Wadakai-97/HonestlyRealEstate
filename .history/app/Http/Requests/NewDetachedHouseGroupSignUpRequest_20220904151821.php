    ...
    public function rules()
    {
        return [
            ...
            'highest_building_coverage_ratio' => ['required', 'integer', 'regex:/^[!-~]+$/', 'digits_between:2,3',
                function($attribute, $value, $fail) {
                    $request = $this->all();
                    if (!empty($request['lowest_building_coverage']) && !empty($request['highest_building_coverage'])) {
                        if(($request['highest_building_coverage'] - $request['lowest_building_coverage']) < 0) {
                            $fail('最高容積率は最低容積率よりも大きい面積、もしくは等しい面積を入力してください。');
                        }
                    }
                }
            ],
