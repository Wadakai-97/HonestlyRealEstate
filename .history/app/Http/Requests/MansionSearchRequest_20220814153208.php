...
class MansionSearchRequest extends FormRequest
{
    ...
    {
        $validator->sometimes('lowest_price' | 'highest_price', 'required | max', function ($input) {
            return $input->lowest_price >= $input->highest_price;
        });

        $validator->sometimes('lowest_occupation_area' | 'highest_occupation_area', 'required | max', function ($input) {
