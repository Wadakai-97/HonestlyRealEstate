...
class MansionSearchRequest extends FormRequest
{
    ...
    {
        $validator->sometimes('lowest_price', 'required | min', function ($input) {
            return $input->lowest < $input->highest_price;
        });

        $validator->sometimes('file_name', 'required | min', function ($input) {
            return $input->type === 'B';
        });
    }
}
