...
class NewDetachedHouseGroupSignUpRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            function($attribute, $value, $fail) {
                $request = $this->all();
                if (!empty($request['highest_price']) && !empty($request['lowest_price']))) {
                ($request['highest_price'] - $request['lowest_price']) < 0) {
    ...
    public function messages() {
        return [
            ...
            'highest_price.min:lowest_price' => '最高価格は最低価格より大きい金額を入力してください。',
