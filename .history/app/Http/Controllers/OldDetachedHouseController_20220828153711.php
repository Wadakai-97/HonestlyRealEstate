...
class OldDetachedHouseController extends Controller
{
    ...
    public function search(DetachedHouseSearchRequest $request) {
        $old_detached_houses = OldDetachedHouse::select('id', 'name', 'price', 'pref', 'municipalities', 'land_area', 'building_area', 'number_of_rooms', 'type_of_room', 'year', 'land_right', 'station', 'access', 'distance_station', 'updated_at')
                                                ->wherePlan($request)
                                                ->wherePref($request)
