...
class LandController extends Controller
{
    ...
    public function imageSignUp($id, LandImageSignUpRequest $request) {
    ...
    public function imageUpdate($id, Request $request) {
        ...
        $_image->imageUpdate($id, $request);
        $ = ::find($_image->_id);
        $_images = Image::where('_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin..edit_image', compact('', '_images','id', 'image_counter'));
    }
    public function list() {
        $lands = Land::select('id', 'name', 'land_area', 'land_right', 'price', 'pref', 'municipalities', 'block')
                    ->paginate(10);
