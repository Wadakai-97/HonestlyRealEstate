...
class NewDetachedHouseController extends Controller
{
    ...
    public function list() {
        $new_detached_houses = NewDetachedHouse::select('id', 'pref', 'municipalities', 'block', 'number_of_rooms', 'type_of_room', 'price', 'land_area', 'building_area', 'land_right')
                            ->paginate(10);

        return view('admin.new_detached_house.list', compact('new_detached_houses'));
    }
    public function adminSearch(Request $request) {
        $new_detached_houses = NewDetachedHouse::select('id', 'pref', 'municipalities', 'block', 'number_of_rooms', 'type_of_room',  'price', 'land_area', 'building_area', 'station', 'walking_distance_station', 'land_right')
                                                ->wherePlan($request)
                                                ->wherePref($request)
                                                ->whereMunicipalities($request)
                                                ->whereLowestPrice($request)
