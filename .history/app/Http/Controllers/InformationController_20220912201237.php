    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            ...
            if(Auth::check()) {
                ...
                if($type = "mansion") {
                    $target = Favorite::where($request->whereMansion($id)->get();
                } else if($type = "land") {
                    $target = Favorite::where($request->whereLand($id)->get();
                } else if($type = "new_detached_house") {
                    $target  = Favorite::where($request->whereNewDetachedHouse($id)->get();
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::where($request->whereNewDetachedHouseGroup($id)->get();
                } else if($type = "old_detached_house") {
                    $target  = Favorite::where($request->whereOldDetachedHouse($id)->get();
                }
            }
