...
class NewDetachedHouseController extends Controller
{
    ...
    public function search(DetachedHouseSearchRequest $request) {
        $new_detached_house = NewDetachedHouse::from('new_detached_houses as A')
                                            ->select('A.id', 'A.name', 'A.pref', 'A.municipalities', 'A.price', DB::raw(" '' AS A.lowest_price"), DB::raw(" '' AS A.highest_price"))
                                            ->wherePlan($request)
                                            ->wherePref($request)
        ...
        $new_detached_houses = NewDetachedHouseGroup::from('new_detached_house_groups as B')
                                                    ->select('B.id', 'B.name', 'B.pref', 'B.municipalities', DB::raw(" '' AS B.price"), 'B.lowest_price', 'B.highest_price')
                                                    ->wherePref($request)
                                                    ->whereMunicipalities($request)
                                                    ->whereLowestPrice($request)
                                                    ->whereHighestPrice($request)
