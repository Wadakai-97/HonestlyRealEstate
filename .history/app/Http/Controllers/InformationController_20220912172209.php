    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            ...
            if($type = "mansion") {
                ...
                $targetouse_id  = Favorite::where('new_detached_house_id', '=', $id)->get();
            } else if($type = "new_detached_house_group") {
                $targetouse_group_id  = Favorite::where('new_detached_house_id', '=', $id)->get();
            } else if($type = "old_detached_house") {
                $targetouse_id  = Favorite::where('mansinon_id', '=', $id)->get();
            }
            $mansion->delete();
        });
