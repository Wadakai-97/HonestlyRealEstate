...
class LandController extends Controller
{
    ...
    public function search(LandSearchRequest $request) {
        $lands = Land::select('id', 'name', 'pref', 'municipalities', 'block', 'price', 'land_area', 'building_coverage_ratio', 'floor_area_ratio', 'land_right', 'construction_conditions', 'station', 'access_method', 'distance_station')
                    ->wherePref($request)
    ...
    public function sort(request $request) {
        ...
        $lands = Land::select('id', 'name', 'pref', 'municipalities', 'block', 'price', 'land_area', 'building_coverage_ratio', 'floor_area_ratio', 'land_right', 'station', 'construction_conditions', 'station', 'elementary_school_name', 'junior_high_school_name', 'access_method', 'distance_station', 'terms_and_conditions', 'property_introduction', 'sales_comment',)
                    ->wherePref($request)
                    ->whereAddress($request)
                    ->whereLowestPrice($request)
                    ->whereHighestPrice($request)
    ...
    public function csvDownload() {
        ...
        $item_name = [
            'ID', '土地名', '販売価格', '税金', '都道府県', '市区町村', '番町番地', '建築条件', '土地面積', '建ぺい率', '容積率', '最寄り駅', '最寄り駅までのアクセス方法', '最寄り駅までの所要時間', '土地権利', 'その他の費用', '都市計画', '用途地域', '法令上の制限', '国土法届出', '地目', '地勢', '接道', '接道幅', '私道', 'セットバック', 'セットバック幅', '上水道', '下水道', 'ガス', '現況', '引き渡し日', '小学校', '小学校までの徒歩距離', '中学校', '中学校までの徒歩距離', '設備条件', '取引態様', '物件紹介コメント', 'セールスコメント', '作成日', '更新日',
        ];
        return $lands->exportCsv($file_name, $item_name);
    }

    public function filteringCsvDownload(Request $request) {
        $lands = Land::whereAddress($request)
                        ->whereLandRight($request)
                        ->whereLowestPrice($request)
                        ->whereHighestPrice($request)
                        ->whereLowestLandArea($request)
