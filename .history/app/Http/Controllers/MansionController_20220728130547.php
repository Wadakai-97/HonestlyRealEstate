...
class MansionController extends Controller
{
    ...
    public function editImage($id) {
        $mansion = Mansion::find($id);
        $mansion_images = MansionImage::where('mansion_id', '=', $id)->get();
        $ex = MansionImage::where('mansion_id', '=', $id)->count();
        $image_counter = 21 - $mansion_images;
        $image_counter = 1;
    ...
    public function mansionImageSignUp($id, Request $request) {
        ...
        $mansion = Mansion::find($id);
        $mansion_images = MansionImage::where('mansion_id', '=', $id)->count();
        $image_counter = 21 - $mansion_images;
        return view('admin.mansion.edit_image', compact('mansion', 'image_counter'))->with('message', '画像登録が完了しました。');
    }
    //Search
    ...
    public function mansionImageUpdate($id, $request) {
        ...
        $request->session()->regenerateToken();
        return redirect()->route('admin.mansion.edit_image', ['id' => $mansion->id])->with('message', '変更内容を登録しました。');
    }
