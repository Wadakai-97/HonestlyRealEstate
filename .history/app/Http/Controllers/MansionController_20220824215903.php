...
class MansionController extends Controller
{
    ...
    public function adminSearch(MansionSearchRequest $request) {
        $mansions = Mansion::select('id', 'pref', 'municipalities',  'apartment_name', 'room', 'price', 'occupation_area', 'number_of_rooms', 'type_of_room', 'year', 'station', 'access_method', 'walking_distance_station')
                            ->wherePlan($request)
                            ->whereAddress($request)
