...
class OldDetachedHouseController extends Controller
{
    ...
    public function imageSignUp($id, DetachedHouseImageSignUpRequest $request) {
        $old_image = new NewDetachedHouseImage;
        $old_image->imageSignUp($id, $request);
        $old = NewDetachedHouse::find($id);
        $old_images = NewDetachedHouseImage::where('old_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.old.edit_image', compact('old', 'image_counter', 'old_images'))->with('message', '画像登録が完了しました。');
    }
    public function imageDelete($id) {
        $old_image = NewDetachedHouseImage::find($id);
        $old_id = $old_image->old_id;

        DB::transaction(function() use($id) {
            $old_image = NewDetachedHouseImage::find($id);
            Storage::delete('storage/property_images/old/' . $old_image->path);
            $old_image->delete();
        });

        $old = NewDetachedHouse::find($old_id);
        $old_images = NewDetachedHouseImage::where('old_id', '=', $old_id)->get();
        $existing_images = NewDetachedHouseImage::where('old_id', '=', $old_id)->count();
        ...
        return view('admin.old.edit_image', compact('old', 'old_images', 'image_counter'));
    }
    public function imageUpdate($id, Request $request) {
        $old_image = NewDetachedHouseImage::find($id);
        $old_image->imageUpdate($id, $request);
        $old = NewDetachedHouse::find($old_image->old_id);
        $old_images = NewDetachedHouseImage::where('old_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.old.edit_image', compact('old', 'old_images','id', 'image_counter'));
    }
