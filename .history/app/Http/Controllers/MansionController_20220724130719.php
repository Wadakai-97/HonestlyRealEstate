...
class MansionController extends Controller
{
    ...
    public function search(MansionSearchRequest $request) {
        $mansions = Mansion::select('pref', 'price', 'occupation_area', 'number_of_rooms', 'years', 'station', 'walking_distance_station')
                            ->wherePref($request)
        ...
        // if(!empty($pref)) {
        //     $mansion = $mansion->Where(function ($mansion) use($pref) {
        ...
        //     $mansion = $mansion->where('price', '>', $lowest_price);
        // }
        // if(!empty($highest_price)) {
        //     $mansion = $mansion->where('price', '<', $highest_price);
        // }
        // if(!empty($lowest_occupation_area)) {
        //     $mansion = $mansion->where('occupation_area', '>=', $lowest_occupation_area);
        // }
        // if(!empty($highest_occupation_area)) {
        //     $mansion = $mansion->where('occupation_area', '<', $highest_occupation_area);
        // }
        // if(!empty($plan)) {
        //     $mansion = $mansion->Where(function ($mansion) use($plan) {
        //         for ($i = 0; $i < count($plan); $i++){
        //                 $mansion->where('number_of_rooms', '=', $plan[$i]);
        //         }
        //     });
        // }
        // if(!empty($old)) {
        //     $date = Carbon::today()->subYear($old);
        //     $mansion = $mansion->whereDate('year', '>', $date);
        // }
        // if(!empty($walking_distance_station )) {
        //     $mansion = $mansion->where('walking_distance_station', '<', $walking_distance_station );
        // }

        $today = Carbon::now()->year;
        $mansions = $mansion->sortable()->paginate(15);
