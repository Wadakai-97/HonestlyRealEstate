...
class MansionController extends Controller
{
    ...
    public function unfavorite($id, Request $request){
        if(Auth::check()) {
            $user = Auth::user()->id;
            $favorite->user_id = Auth::user()->id;
            $favorite = Favorite::where('mansion_id', $id)->where('user_id', $user)->get();
        } else {
            $user = $request->ip();
            $favorite = Favorite::where('mansion_id', $id)->where('ip', $user)->get();
        }
        $favorite->delete();
        return back();
    }
    ...
    public function kanagawaSearch(MansionSearchRequest $request) {
        ...
        $favorite = Favorite::where('type', '=', 'mansion')->where('property_id', '=', $request->id);

        return view('user.mansion.kanagawa.result', compact('today', 'mansions', 'request', 'favorite'));
    }
