...
class NewDetachedHouseController extends Controller
{
    ...
    public function save($id) {
        DB::transaction(function() use($id, $request) {
            $ex_old_detached_house = old_detached_house::find($id);
            $ex_old_detached_house->apartment_name = $request->apartment_name;
            ...
            if(!empty($request->file('files'))) {
                foreach ($request->file('files') as $index => $e) {
                    ...
                    $images = old_detached_houseImage::find($id);
                    $images->path = $file_name;
        ...
        $old_detached_house = old_detached_house::find($id);
        $request->session()->regenerateToken();
