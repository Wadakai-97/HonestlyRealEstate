...
class NewDetachedHouseController extends Controller
{
    ...
    public function search(DetachedHouseSearchRequest $request) {
        ...
        $new_detached_houses = NewDetachedHouseGroup::from('new_detached_house_groups')
                                                    ->select('id', 'name', 'pref', 'municipalities', DB::raw(" '' AS price"), 'lowest_price', 'highest_price', DB::raw(" '' AS land_area"), 'lowest_land_area', 'highest_land_area', DB::raw(" '' AS building_area"), 'lowest_building_area', 'highest_building_area', DB::raw(" '' AS number_of_rooms"), 'lowest_number_of_rooms', 'highest_number_of_rooms', DB::raw(" '' AS type_of_room"), 'lowest_type_of_room', 'highest_type_of_room', 'station', 'walking_distance_station', 'land_right', 'updated_at')
                                                    ->wherePref($request)
                                                    ->whereMunicipalities($request)
                                                    ->whereLowestPrice($request)
                                                    ->whereHighestPrice($request)
                                                    ...
                                                    ->whereWalkingDistanceStation($request)
                                                    ->whereLandRight($request)
                                                    ->UnionAll($new_detached_house)
                                                    ->get();

        $page = $request->get('page', 1);
        $paginate = 5;
        $new_detached_houses = $this->paginate($new_detached_houses);
        $new_detached_houses->withPath('/results');

        // $page = $request->get('page', 1);
        // $paginate = 15;

        // $offset = ($page * $paginate) - $paginate;
        // $current_page = array_slice($new_detached_houses, $offset, $paginate, true);
        // $new_detached_houses = new LengthAwarePaginator($current_page, count($new_detached_houses), $paginate, $page);

        return view('user.new_detached_house.result', compact('new_detached_houses', 'request'));
    }
    public function paginate($new_detached_house, $perPage = 5, $page = null, $options = [])
    {
        $page = $page ?: (Paginator::resolveCurrentPage() ?: 1);
        $new_detached_house = $new_detached_house instanceof Collection ? $new_detached_house : Collection::make($new_detached_house);
        return new LengthAwarePaginator($new_detached_house->forPage($page, $perPage), $new_detached_house->count(), $perPage, $page, $options);
    }
    ...
    public function list() {
        $new_detached_houses = NewDetachedHouse::select('id', 'pref', 'municipalities', 'block', 'number_of_rooms', 'type_of_room', 'price', 'land_right', 'land_area', 'building_area', 'station', 'walking_distance_station')
                            ->paginate(15);
    ...
    public function groupList() {
        $new_detached_house_groups = NewDetachedHouseGroup::select('id', 'pref', 'municipalities', 'block', 'lowest_number_of_rooms', 'lowest_type_of_room', 'highest_number_of_rooms',  'highest_type_of_room', 'lowest_price', 'highest_price', 'lowest_land_area', 'highest_land_area', 'lowest_building_area', 'highest_building_area', 'land_right', 'station', 'walking_distance_station')
                                                        ->paginate(15);
