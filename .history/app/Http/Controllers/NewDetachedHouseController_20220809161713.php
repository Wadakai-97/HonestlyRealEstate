...
class NewDetachedHouseController extends Controller
{
    ...
    // For Image
    public function imageSignUp($id, DetachedHouseImageSignUpRequest $request) {
    ...
    public function groupUpdate($id, NewDetachedHouseGroupSignUpRequest $request) {
      ...
      // For Image
    public function groupImageSignUp($id, DetachedHouseGroupImageSignUpRequest $request) {
        $new_detached_image = new NewDetachedHouseGroupImage;
        $new_detached_image->imageSignUp($id, $request);
        $new_detached = NewDetachedHouseGroup::find($id);
        $new_detached_images = NewDetachedHouseGroupImage::where('new_detached_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.new_detached.edit_image', compact('new_detached', 'image_counter', 'new_detached_images'))->with('message', '画像登録が完了しました。');
    }
    public function groupImageDelete($id) {
        $new_detached_image = NewDetachedHouseGroupImage::find($id);
        $new_detached_id = $new_detached_image->new_detached_id;

        DB::transaction(function() use($id) {
            $new_detached_image = NewDetachedHouseGroupImage::find($id);
            Storage::delete('storage/property_images/new_detached/' . $new_detached_image->path);
            $new_detached_image->delete();
        });

        $new_detached = NewDetachedHouseGroup::find($new_detached_id);
        $new_detached_images = NewDetachedHouseGroupImage::where('new_detached_id', '=', $new_detached_id)->get();
        $existing_images = NewDetachedHouseGroupImage::where('new_detached_id', '=', $new_detached_id)->count();
        ...
        return view('admin.new_detached.edit_image', compact('new_detached', 'new_detached_images', 'image_counter'));
    }
    public function groupImageUpdate($id, Request $request) {
        $new_detached_image = NewDetachedHouseGroupImage::find($id);
        $new_detached_image->imageUpdate($id, $request);
        $new_detached = NewDetachedHouseGroup::find($new_detached_image->new_detached_id);
        $new_detached_images = NewDetachedHouseGroupImage::where('new_detached_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.new_detached.edit_image', compact('new_detached', 'new_detached_images','id', 'image_counter'));
    }
}
