...
class MansionController extends Controller
{
    ...
    public function search(Request $request) {
        ...
        $results = Mansion::select('id', 'apartment_name', 'pref', 'municipalities', 'price', 'occupation_area', 'number_of_rooms', 'type_of_room', 'floor', 'story', 'year', 'station', 'walking_distance_station', 'updated_at')
                            ->wherePref($pref)
        ...
        $number_of_result = $mansions->count();
        $mansions = $mansions->paginate(15);
        $today = Carbon::today()->year;
        $request->session()->regenerateToken();
        return view('user.mansion.result', compact('today', 'mansions', 'request', 'number_of_result'));
    }
