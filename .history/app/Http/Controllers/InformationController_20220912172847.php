    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            ...
            if(Auth::check()) {
                ...
                if($type = "mansion") {
                    $target = Favorite::where('user_id', '=', $user)->whereMansion($id)->get();
                } else if($type = "land") {
                    $target = Favorite::where('user_id', '=', $user)->whereLand($id)->get();
                } else if($type = "new_detached_house") {
                    $target  = Favorite::whereNewDetachedHouse($id)->get();
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::whereNewDetachedHouseGroup($id)->get();
                } else if($type = "old_detached_house") {
                    $target  = Favorite::whereOldDetachedHouse($id)->get();
                }
            } else {
                if($type = "mansion") {
                    ...
                    $target  = Favorite::where('new_detached_house_group_id', '=', $id)->get();
                } else if($type = "old_detached_house") {
                    $target  = Favorite::where('old_detached_house_id', '=', $id)->get();
                }
            }
            $mansion->delete();
        });
