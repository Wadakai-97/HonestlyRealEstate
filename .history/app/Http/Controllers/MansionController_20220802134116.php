...
class MansionController extends Controller
{
    ...
    public function detail($id) {
        ...
        $similar_mansion = new MansionAccess;

        if(is_null($similar_mansion->similar_mansions_updated_at) || now()->diffInDays($similar_mansion->similar_mansions_updated_at) >= 3) {
            $id = $similar_mansion->id;
            $ip_addresses = $similar_mansion->accesses->pluck('ip');
            ...
            $access_counts = $mansions->groupBy('mansion_id')
                                        ...
                                        ->sortDesc();
            $similar_mansion->similar_mansions()->delete();
            ...
            $similar_mansion->simiilar_mansions_updated_at = now();
            $similar_mansion->save();
        }

        return view('user.mansion.detail', compact('mansion', 'mansions', 'today', 'rollover_date'));
    }
