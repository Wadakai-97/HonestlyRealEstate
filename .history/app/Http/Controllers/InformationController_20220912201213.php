    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            ...
            if(Auth::check()) {
                ...
                if($type = "mansion") {
                    $target = Favorite::whereGuest()->whereMansion($id)->get();
                } else if($type = "land") {
                    $target = Favorite::whereGuest()->whereLand($id)->get();
                } else if($type = "new_detached_house") {
                    $target  = Favorite::whereGuest()->whereNewDetachedHouse($id)->get();
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::whereGuest()->whereNewDetachedHouseGroup($id)->get();
                } else if($type = "old_detached_house") {
                    $target  = Favorite::whereGuest()->whereOldDetachedHouse($id)->get();
                }
            }
