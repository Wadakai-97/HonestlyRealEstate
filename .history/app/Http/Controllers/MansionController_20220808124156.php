...
class MansionController extends Controller
{
    ...
    public function imageDelete($id) {
        DB::transaction(function() use($id) {
            ...
            $mansion = $mansion_image->mansion_id;
            Storage::delete('/storage/property_images/mansion' . $mansion_image->path);
            $mansion_image->delete();
        });
        $mansion_images = MansionImage::where('mansion_id', '=', $mansion->id)->get();
        $existing_images = MansionImage::where('mansion_id', '=', $mansion->id)->count();
        if(!empty($existing_images)) {
            $image_counter = $existing_images;
        } else if(empty($existing_images)) {
            $image_counter = 1;
        }
        return view('admin.mansion.edit_image', compact('mansion', 'mansion_images', 'image_counter'))->with('message', '削除に成功しました。');
    }
    ...
    public function imageUpdate($id, Request $request) {
        ...
        $request->session()->regenerateToken();
        return view('admin.mansionImage.edit', compact('mansion', 'mansion_images', 'image_counter', 'id'))->with('message', '変更内容を登録しました。');
    }
    // For Recommend
