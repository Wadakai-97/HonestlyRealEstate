    ...
    public function keywordSearch(Request $request) {
        ...
        if($type == "新築戸建") {
            ...
            $old_detached_houses = OldDetachedHouse::select('id', 'name', 'price', 'pref', 'municipalities', 'land_area', 'building_area', 'number_of_rooms', 'type_of_room', 'year', 'land_right', 'station', 'access_method', 'distance_station')
                                                    ->whereKeyword($request)
                                                    ->paginate(15);
            return view('user.old_detached_house.result', compact('today', 'old_detached_houses', 'request'));
        } else if($type == "マンション") {
            ...
            $lands = Land::select('id', 'apartment_name', 'pref', 'municipalities', 'price', 'occupation_area', 'number_of_rooms', 'type_of_room', 'floor', 'story', 'year', 'station', 'access_method', 'distance_station')
                        ->whereKeyword($request)
                        ->paginate(15);
            return view('user.land.result', compact('today', 'lands', 'request'));
        }
    }
