...
class MansionController extends Controller
{
    ...
    public function detail($id) {
        ...
        $similar_mansion = MansionAccess::select('mansion_id')->where('mansion_id', '=', $id)->get();
        d($similar_mansion)

        if(!empty($similar_mansion->updated_at) || now()->diffInDays($similar_mansion->updated_at) >= 3) {
            $id = $similar_mansion->id;
            $ip_addresses = $similar_mansion->mansion_accesses->pluck('ip');
            ...
            $similar_mansion->updated_at = now();
            $similar_mansion->save();
        }
