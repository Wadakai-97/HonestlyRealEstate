...
class InformationController extends Controller
{
    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            if(Auth::check()) {
                $favorite = Favorite::whereGuest()->get();
                if($type = "mansion") {
                    $target = Favorite::whereUser()->whereMansion($id);
                } else if($type = "land") {
                    $target = Favorite::whereUser()->whereLand($id);
                } else if($type = "new_detached_house") {
                    $target  = Favorite::whereUser()->whereNewDetachedHouse($id);
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::whereUser()->whereNewDetachedHouseGroup($id);
                } else if($type = "old_detached_house") {
                    $target  = Favorite::whereUser()->whereOldDetachedHouse($id);
                }
            } else {
                $favorite = Favorite::whereGuest($request)->get();
                if($type = "mansion") {
                    $target = Favorite::whereGuest($request)->whereMansion($id);
                } else if($type = "land") {
                    $target = Favorite::whereGuest($request)->whereLand($id);
                } else if($type = "new_detached_house") {
                    $target  = Favorite::whereGuest($request)->whereNewDetachedHouse($id);
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::whereGuest($request)->whereNewDetachedHouseGroup($id);
                } else if($type = "old_detached_house") {
                    $target  = Favorite::whereGuest($request)->whereOldDetachedHouse($id);
                }
            }
            if(!empty($target)) {
                $target->delete();
