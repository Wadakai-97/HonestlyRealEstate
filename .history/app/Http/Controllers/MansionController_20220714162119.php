...
class MansionController extends Controller
{
    ...
    public function adminSearch(Request $request) {
        $apartment_name_keyword = $request->apartment_keyword;
        $municipalities_keyword = $request->municipalities_keyword;
        $query = Mansion::query();

        if(!empty($apartment_keyword)) {
            $mansions = Mansion::searchbyapartmentname($apartment_name_keyword)->get();
        }
        return view('admin.mansion.list');
    }
