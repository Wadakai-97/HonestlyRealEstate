...
class InformationController extends Controller
{
    ...
    public function list() {
        $newses = Information::select('id', 'title', 'body', 'staff_id', 'created_at')
                            ->paginate(10);
        $staffs = Staff::get();
    ...
    public function adminSearch(Request $request) {
        $newses = Information::select('id', 'land_area', 'land_right', 'price', 'pref', 'municipalities', 'block', 'construction_conditions', 'station', 'walking_distance_station')
                    ->whereAddress($request)
                    ->paginate(15);

        $request->session()->regenerateToken();

        return view('admin.land.result', compact('newses', 'request'));
    }
