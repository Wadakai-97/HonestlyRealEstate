    ...
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            if(Auth::check()) {
                $favorite = Favorite::whereUser()->get();
                if($type = "mansion") {
                    $target = $favo->whereMansion($id);
                } else if($type = "land") {
                    $target = $favo->whereLand($id);
                } else if($type = "new_detached_house") {
                    $target  = $favo->whereNewDetachedHouse($id);
                } else if($type = "new_detached_house_group") {
                    $target  = $favo->whereNewDetachedHouseGroup($id);
                } else if($type = "old_detached_house") {
                    $target  = $favo->whereOldDetachedHouse($id);
                }
            } else {
                $favorite = Favorite::whereGuest($request)->get();
                if($type = "mansion") {
