...
class InformationController extends Controller
{
    ...
    public function recommendDelete($id, Request $request) {
        DB::transaction(function() use($id, $request) {
            if(!empty($request->land)) {
                $recommend_property = Recommend::where('land_id', '=', $id)->delete();
            }
            if(!empty($request->mansion)) {
                $recommend_property = Recommend::where('mansion_id', '=', $id)->delete();
            }
            if(!empty($request->new_detached_house)) {
                $recommend_property = Recommend::where('new_detached_house_id', '=', $id)->delete();
            }
            if(!empty($request->new_detached_house_group)) {
                $recommend_property = Recommend::where('new_detached_house_group_id', '=', $id)->delete();
            }
            if(!empty($request->old_detached_house)) {
                $recommend_property = Recommend::where('old_detached_house_id', '=', $id)->delete();
            }
        });
        return redirect()->route('admin.recommend.list')->with('message', 'お気に入り登録を')
    }
