...
class NewDetachedHouseController extends Controller
{
    ...
    public function groupImageSignUp($id, DetachedHouseGroupImageSignUpRequest $request) {
        $new_detached_house_image = new NewDetachedHouseGroupImage;
        $new_detached_house_image->imageSignUp($id, $request);
        $new_detached_house = NewDetachedHouseGroup::find($id);
        $new_detached_house_images = NewDetachedHouseGroupImage::where('new_detached_house_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.new_detached_house.edit_image', compact('new_detached_house', 'image_counter', 'new_detached_house_images'))->with('message', '画像登録が完了しました。');
    }
    public function groupImageDelete($id) {
        $new_detached_house_image = NewDetachedHouseGroupImage::find($id);
        $new_detached_house_id = $new_detached_house_image->new_detached_house_id;
        ...
        $new_detached_house = NewDetachedHouseGroup::find($new_detached_house_id);
        $new_detached_house_images = NewDetachedHouseGroupImage::where('new_detached_house_id', '=', $new_detached_house_id)->get();
        $existing_images = NewDetachedHouseGroupImage::where('new_detached_house_id', '=', $new_detached_house_id)->count();
    ...
    public function groupImageUpdate($id, Request $request) {
        $new_detached_house_image = NewDetachedHouseGroupImage::find($id);
        $new_detached_house_image->imageUpdate($id, $request);
        $new_detached_house = NewDetachedHouseGroup::find($new_detached_house_image->new_detached_house_id);
        $new_detached_house_images = NewDetachedHouseGroupImage::where('new_detached_house_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
        return view('admin.new_detached_house.edit_image', compact('new_detached_house', 'new_detached_house_images','id', 'image_counter'));
