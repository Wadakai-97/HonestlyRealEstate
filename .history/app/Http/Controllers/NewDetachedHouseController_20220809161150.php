...
class NewDetachedHouseController extends Controller
{
    ...
    public function imageSignUp($id, DetachedHouseImageSignUpRequest $request) {
        $new_detached_house_image = new NewDetacheImage;
        $new_detached_house_image->imageSignUp($id, $request);
        $new_detached_house = new_detached_house::find($id);
        $new_detached_house_images = new_detached_houseImage::where('new_detached_house_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
    ...
    public function imageDelete($id) {
        ...
        $new_detached_house = new_detached_house::find($new_detached_house_id);
        $new_detached_house_images = new_detached_houseImage::where('new_detached_house_id', '=', $new_detached_house_id)->get();
        $existing_images = new_detached_houseImage::where('new_detached_house_id', '=', $new_detached_house_id)->count();
    ...
    public function imageUpdate($id, Request $request) {
        ...
        $new_detached_house = new_detached_house::find($new_detached_house_image->new_detached_house_id);
        $new_detached_house_images = new_detached_houseImage::where('new_detached_house_id', '=', $id)->get();
        $image_counter = 1;
        $request->session()->regenerateToken();
