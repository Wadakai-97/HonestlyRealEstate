...
class MansionController extends Controller
{
    ...
    public function adminSearch(Request $request) {
        ...
        if(!empty($municipalities)) {
            $query = $query->Where(function ($query) use($municipalities) {
                for ($i = 0; $i < count($municipalities); $i++){
                    $query->orwhere('municipalities', '=',  $municipalities[$i]);
                }
            });
        }
        if(!empty($municipalities)) {
            $query = $query->where('price', '>', $municipalities);
        }
        if(!empty($lowest_price)) {
            $query = $query->where('price', '>', $lowest_price);
        }
        ...
        return view('admin.mansion.list', compact('mansions', 'request'));
    }
