...
class InformationController extends Controller
{
    ...
    public function favoriteSignUp($id, Request $request){
        ...
        $flash_message = "お気に入り登録に成功しました。";

        return redirect()->route('mansion.result')->with($message, $flash_message);
    }
    // 登録削除
    public function favoriteSignOff($id, Request $request){
        DB::transaction(function() use($id, $request) {
            ...
            if(Auth::check()) {
                ...
                if($type = "mansion") {
                    $target = Favorite::where($request)->whereMansion($id)->delete();
                } else if($type = "land") {
                    $target = Favorite::where($request)->whereLand($id)->delete();
                } else if($type = "new_detached_house") {
                    $target  = Favorite::where($request)->whereNewDetachedHouse($id)->delete();
                } else if($type = "new_detached_house_group") {
                    $target  = Favorite::where($request)->whereNewDetachedHouseGroup($id)->delete();
                } else if($type = "old_detached_house") {
                    $target  = Favorite::where($request)->whereOldDetachedHouse($id)->delete();
                }
            }
