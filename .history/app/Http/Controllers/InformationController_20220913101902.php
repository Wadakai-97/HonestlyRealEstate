...
class InformationController extends Controller
{
    ...
    public function favoriteSignUp($id, Request $request){
        ...
        if($type = "mansion") {
            ...
            $path = "new_detached_house.result";
        } else if($type = "new_detached_house_group") {
            $path = "new_detached_house_group.result";
        } else if($type = "old_detached_house") {
            $path = "old_detached_house.result";
        }
        dd($path);
        return redirect()->route($path)->with('message', 'お気に入り登録に成功しました。');
    }
    // 登録削除
