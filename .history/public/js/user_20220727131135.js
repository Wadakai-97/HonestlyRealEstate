...
// 都道府県セレクトボックス
$(function() {
    // $path = public_path('json/pref_municipalities.json');
    $.getJSON('/public/json/pref_municipalities.json', function(data) {
    for(var i=1; i<48; i++) {
...
$('#pref').on('change', function() {
    // $path = public_path('json/pref_municipalities.json');
    $('#municipalities option:nth-child(n+2)').remove();
        var pref = ('00'+$('#pref option:selected').val()).slice(-2);
        var key = Number(pref)-1;
        $.getJSON('HRE/public/json/pref_municipalities.json', function(data) {
        for(var i=0; i<data[key][pref].municipalities.length; i++){
            $('#municipalities').append('<option value="'+data[key][pref].municipalities[i].id+'">'+data[key][pref].municipalities[i].name+'</option>');
        }
    });
});
