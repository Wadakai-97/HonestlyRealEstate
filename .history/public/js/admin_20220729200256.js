...
function deletePropertyImage(target) {
    var sign_up_form = document.getElementById("signUpForm" + target).innerText('<img src="{{ asset("/storage/mansion_images/no_image.jpeg") }}" id="noImage{{ $image_counter }}" class="no_image" alt="物件画像{{ $image_counter }}">',
                        '<div id="{{ $image_counter }}"></div>',
                        '<div class="form-group @if(!empty($errors->first("image" . $image_counter))) has-error @endif">',
                        '<input type="file" name="image" id="inputImage{{ $image_counter }}" onchange="previewPropertyImage(event, {{ $image_counter }})" value="{{ $mansion_image->path }}">',
                        '<span class="help-block">{{$errors->first("image" . $image_counter )}}</span>',
                        '</div>');
    // var sign_up_form = document.getElementById("signUpForm" + target);
    // var tako = document.getElementById("showImage" + target);
    // var shake = document.getElementById("showImage" + target);
    // console.log(tako);
    // console.log(shake);

    // sign_up_form.empty();
    // sign_up_form.append('<img src="{{ asset("/storage/mansion_images/no_image.jpeg") }}" id="noImage{{ $image_counter }}" class="no_image" alt="物件画像{{ $image_counter }}">',
    //                     '<div id="{{ $image_counter }}"></div>',
    //                     '<div class="form-group @if(!empty($errors->first("image" . $image_counter))) has-error @endif">',
    //                     '<input type="file" name="image" id="inputImage{{ $image_counter }}" onchange="previewPropertyImage(event, {{ $image_counter }})" value="{{ $mansion_image->path }}">',
    //                     '<span class="help-block">{{$errors->first("image" . $image_counter )}}</span>',
