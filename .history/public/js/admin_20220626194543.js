...
// セットバック：メートル表示
$(function() {
    const setback_length = document.querySelector('#setback_length');
    const setback = document.querySelector('#setback');

    $('#setback').keyup(function() {
    setback.addEventListener('click', (e)=>{
        if(e.target.checked){
            setback_length.style.display = 'block';

        } else {
            setback_length.style.display = 'none';
        }
    }, false);
};
...
$(function() {
    $('#countSalesComment').keyup(function() {
        ...
        $('#showSalesComment').text(remain);
            if (remain === 0) {
                $('#showSalesComment').css('color', 'red');
            } else if (remain < 200) {
                $('#showSalesComment').css('color', 'green');
            }
    });
});
// 物件紹介コメント：入力文字数制限
$(function() {
    $('#countIntroComment').keyup(function() {
        ...
        $('#showIntroComment').text(remain);
            if (remain === 0) {
                $('#showIntroComment').css('color', 'red');
            } else if (remain < 200) {
                $('#showIntroComment').css('color', 'green');
            }
    });
});
// 設備条件：入力文字数制限
