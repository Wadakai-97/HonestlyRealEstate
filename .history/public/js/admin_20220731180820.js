...
function deletePropertyImage(target) {
    ...
    sign_up_form.insertAdjacentHTML('afterbegin', '<img src="{{ asset("/storage/mansion_images/no_image.jpeg") }}" id="noImage{{ $image_counter }}" class="no_image" alt="物件画像{{ $image_counter }}"><div id="{{ $image_counter }}"></div><div class="form-group @if(!empty($errors->first("image" . $image_counter))) has-error @endif">
