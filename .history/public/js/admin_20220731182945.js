...
function deletePropertyImage(target) {
    ...
    sign_up_form.insertAdjacentHTML('afterbegin', '"<img src="/public/storage/mansion_images/no_image.jpeg" id="noImage" + target "{{ $image_counter }}" class="no_image" alt="物件画像{{ $image_counter }}"><div id="{{ $image_counter }}"></div><div class="form-group @if(!empty($errors->first("image" . $image_counter))) has-error @endif"><input type="file" name="image" id="inputImage{{ $image_counter }}" onchange="previewPropertyImage(event, {{ $image_counter }})" value="{{ $mansion_image->path }}"><span class="help-block">{{$errors->first("image" . $image_counter )}} {{$image_counter}}</span></div>');
}
// 入力画像プレビュー
