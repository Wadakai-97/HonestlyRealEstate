...
class CreateOldDetachedHousesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('old_detached_houses', function (Blueprint $table) {
            ...
            $table->integer('price')->unsigned()->comment('販売価格');
            ...
            $table->float('land_area', 7, 2)->nullable()->comment('土地面積');
            $table->float('building_area', 7, 2)->nullable()->comment('建物面積');
            $table->string('balcony')->comment('バルコニー');
            $table->float('balcony_area', 5, 2)->nullable()->comment('バルコニー面積');
            ...
            $table->integer('parking_lot')->nullable()->comment('駐車台数');
            ...
            $table->string('adjacent_road')->comment('接道種類');
            $table->string('adjacent_road_direction')->comment('接道方向');
            $table->float('adjacent_road_width', 3, 2)->comment('接道幅');
            $table->string('private_road')->comment('私道');
            $table->string('setback')->comment('セットバック');
            $table->float('setback_length', 3, 2)->nullable()->comment('セットバック幅');
