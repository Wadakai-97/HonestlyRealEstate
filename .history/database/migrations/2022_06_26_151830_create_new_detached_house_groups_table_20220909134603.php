...
class CreateNewDetachedHouseGroupsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('new_detached_house_groups', function (Blueprint $table) {
            ...
            $table->float('lowest_price', 6, 2)->comment('販売価格');
            $table->float('highest_price', 6, 2)->comment('販売価格');
            ...
            $table->float('lowest_adjacent_road_width', 3, 2)->comment('最小接道幅');
            $table->float('highest_adjacent_road_width', 3, 2)->comment('最大接道幅');
            $table->string('private_road')->comment('私道');
            $table->string('setback')->comment('セットバック');
            $table->float('lowest_setback_length', 3, 2)->nullable()->comment('最小セットバック幅');
            $table->float('highest_setback_length', 3, 2)->nullable()->comment('最高セットバック幅');
